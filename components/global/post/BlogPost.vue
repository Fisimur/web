...
export default {
  ...
  async fetch() {
    ...
    const posts = await $content('posts', { deep: true })
      .where({ permalink: `${this.year}/${this.month}/${this.day}/${this.permalink}` })
      .fetch()
    const [post] = posts
    ...
    if (!post) {
      ...
      this.error.message = 'No hemos encontrado el artículo que buscas'
  ...
  computed: {
    ...
    blogPostImage() {
      const postImage = this.post.image
